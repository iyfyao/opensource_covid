...
if life_death == 'Total Death' :
    if len(selected_country) > 0:
            fig = px.line(df[df['location'].isin(selected_country)], x='date', y='total_deaths',range_x = [date_start,date_end], color = 'location', title = "Total number of cases Line plot")

if life_death == 'Total Cases':
    if len(selected_country) > 0:
            fig = px.line(df[df['location'].isin(selected_country)], x='date', y='total_cases',range_x = [date_start,date_end], color = 'location', title = "Total number of cases Line plot")
